...
export const GalleryPage = (_props: GalleryPageProps): React.JSX.Element => {
  ...
  const handleConfirmMoveDialog = async (folderId: number | null, itemIds: string[]): Promise<void> => {
    ...
    try {
      ...
      if (folderId !== null) { setSelectedViewScope('folder'); setSelectedFolderId(folderId); }
      const refreshed = await refresh();
      if (!refreshed.ok) { toast.error(refreshed.error); return; }
    ...
    >
      ...
      <div style={{ borderBottom: `1px solid ${T.line}`, flexShrink: 0 }}>
        <GalleryToolbar
          ...
          selectedFolderName={selectedFolderId !== null ? (findFolderNameById(folders, selectedFolderId) ?? 'Folder') : null}
